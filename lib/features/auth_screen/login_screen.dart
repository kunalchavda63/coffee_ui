            ...
            return BlocConsumer<LoginBloc,LoginState>(
              listener: (context,state){
                if(state is LoginSuccess){
                  logger.i('Login Success : ${state.data}');
                } else if(state is LoginFailure){
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: CustomWidgets.customText(data: state.error)));
                }
              },
                        ...
                        child: Column(
                          children: [
                            CustomWidgets.customText(
                                ...
                                style: const TextStyle().family(FontFamily.sen).s(30.sp),
                            ).padTop(118.r),
                            Opacity(
                              opacity: 0.8,
                    ...
                    Positioned(
                        ...
                        child: CustomWidgets.customAnimationWrapper(
                          ...
                          child: SvgPicture.asset(
                            ...
                            colorFilter: const ColorFilter.mode(AppColors.hex5e61,BlendMode.srcIn),
                          ),
                        ),),
                                ...
                                Row(
                                  children: [
                                    ...
                                    CustomWidgets.customText(
                                        ...
                                        style: const TextStyle().s(13.sp).c(AppColors.hex7e8a).family(FontFamily.sen),
                                    ),
                                    const Spacer(),
                                    GestureDetector(
                                      onTap: (){
                                         getIt<AppRouter>().push<dynamic>(const ForgotScreen());
                                        logger.i('Pushing : ${getIt<AppRouter>().navigatorKey.currentWidget}');
                                      },
                                      child: CustomWidgets.customText(
                                          data: AppStrings.forgotPassword,
                                          style: const TextStyle().s(14.sp).c(AppColors.hexFf76).family(FontFamily.sen),
                                      ),
                                    ),
                                  ],
                                ).padBottom(29.r),
                                  ...
                                  child: CustomWidgets.customButton(
                                      onTap: () async {
                                        ...
                                        if(permission == LocationPermission.denied || permission==LocationPermission.deniedForever){
                                          getIt<AppRouter>().push<dynamic>(const AccessLocationScreen());
                                        }
                                        else if(permission == LocationPermission.always || permission == LocationPermission.whileInUse){
                                          if(_form.currentState!.validate()) {
                                            if(!context.mounted){return;}
                                           context.read<LoginBloc>().add(
                                              LoginSubmitted(email: _emailController.text.trim() , password: _passController.text.trim()),
                                            );
                                  ...
                                  children: [
                                    ...
                                    InkWell(
                                      onTap: (){
                                        ...
                                        getIt<AppRouter>().push<dynamic>(const SignUpScreen());
                                      },
                                      child: CustomWidgets.customText(
